...
public class ResourceHandler implements HttpHandler {
     ...
     */
    private final HashMap<Integer, HttpHandler> exceptionHandlres = new HashMap<>();

    public ResourceHandler(ResourceManager resourceManager) {
        this(resourceManager, ResponseCodeHandler.HANDLE_404);
    }

    public ResourceHandler(ResourceManager resourceManager, HttpHandler handle404) {
        this.resourceManager = resourceManager;
        exceptionHandlres.put(StatusCodes.NOT_FOUND, handle404);
    }
    ...
    public ResourceHandler() {
        exceptionHandlres.put(StatusCodes.NOT_FOUND, ResponseCodeHandler.HANDLE_404);
    }
    ...
    public void addExceptionHandler (Integer statusCode, HttpHandler handler) {
    	exceptionHandlres.put(statusCode, handler);
    }
        ...
        HttpHandler dispatchTask = new HttpHandler() {
            ...
            public void handleRequest(HttpServerExchange exchange) throws Exception {
                ...
                if (resource == null) {
                	...
                	handleExceptionHandler(StatusCodes.NOT_FOUND, exchange);
                    return;
                }
                ...
                if (resource.isDirectory()) {
                    ...
                    if (indexResource == null) {
                        if (directoryListingEnabled) {
                        	...
                        	handleExceptionHandler(StatusCodes.FORBIDDEN, exchange);
                            return;
                        }
    ...
    protected void handleExceptionHandler(int statusCode, HttpServerExchange exchange) throws Exception {
        HttpHandler httpHandler = exceptionHandlres.get(statusCode);

        if (httpHandler != null) {
        	exchange.setStatusCode(statusCode);
        	httpHandler.handleRequest(exchange);
        } else if (StatusCodes.NOT_FOUND == statusCode) {
        	ResponseCodeHandler.HANDLE_404.handleRequest(exchange);
        } else if (StatusCodes.FORBIDDEN == statusCode) {
        	exchange.setStatusCode(StatusCodes.FORBIDDEN);
            exchange.endExchange();
        }
	}
